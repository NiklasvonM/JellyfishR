...
fn vec_levenshtein_distance<T: PartialEq>(v1: &FastVec<T>, v2: &FastVec<T>) -> usize {
    let rows = v1.len() + 1;
    let cols = v2.len() + 1;
...
fn vec_damerau_levenshtein_distance<T: Eq + std::hash::Hash>(
    v1: &FastVec<T>,
    v2: &FastVec<T>,
) -> usize {
    let len1 = v1.len();
...
pub fn levenshtein_distance_(s1: &str, s2: &str) -> usize {
    if s1 == s2 {
        return 0;
    }
...
pub fn damerau_levenshtein_distance_(s1: &str, s2: &str) -> usize {
    if s1 == s2 {
        return 0;
    }
...
mod test {
    ...
    #[test]
    fn test_levenshtein() {
        testutils::test_distance_func("testdata/levenshtein.csv", levenshtein_distance_);
    }
